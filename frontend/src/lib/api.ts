...
async function request<T>(path: string, init?: RequestOptions): Promise<T> {
  const { quiet, responseType = 'json', timeoutMs = DEFAULT_REQUEST_TIMEOUT_MS, ...fetchInit } = init ?? {}
...
export interface AiStockReport {
  ...
  levels?: Record<LevelType, PriceLevel[]>
  created_at: string
  source?: 'portfolio' | 'daily_review'
  source_ref?: string
  daily_review_date?: string
}
...
/** 后端响应另含 profile 费率快照, 录单只需 fee/tax, 这里有意收窄 */
export interface TradeEstimateResult {
  fee: number
  tax: number
}

export type StatementPreviewMode = 'insert' | 'calibrate' | 'skip'

/** current_* 仅 mode 为 calibrate/skip (匹配到既有交易) 时存在 */
...
export interface StatementPreviewResponse {
  ...
  unresolved: { raw_code: string; name: string; trade_date: string }[]
}

export interface StatementCommitItem {
  mode: 'insert' | 'calibrate'
  matched_trade_id?: string | null
  symbol?: string
  trade_date?: string
  side?: 'buy' | 'sell'
...
export interface PortfolioPriceMonitor {
  symbol: string
  name: string
  asset_type: 'stock' | 'etf'
  stop_loss_price: number | null
  stop_loss_enabled: boolean
  add_position_price: number | null
  add_position_enabled: boolean
  webhook_channels: string[]
}

export interface PortfolioAccountSnapshot extends PortfolioAccount {
...
export type DailyReviewItemStatus = 'pending' | 'running' | 'completed' | 'failed' | 'interrupted' | 'blocked'
export type DailyReviewStatus = 'running' | 'completed' | 'degraded' | 'failed' | 'interrupted'
export type DailyReviewGraphNodeStatus = DailyReviewItemStatus | 'blocked'
export type DailyReviewGraphEdgeStatus = 'pending' | 'ready' | 'active' | 'completed' | 'failed' | 'blocked'
export const DAILY_REVIEW_GRAPH_SCHEMA_VERSION = 4

export interface DailyReviewGraphInputField {
  key: string
  label: string
  value: unknown
}

export interface DailyReviewGraphUpstreamInput {
  node_id: string
  label: string
  status: DailyReviewGraphNodeStatus
  summary: string
}

export interface DailyReviewGraphNodeInput {
  summary: string
  facts_summary: string
  fields: DailyReviewGraphInputField[]
  upstream: DailyReviewGraphUpstreamInput[]
  debate?: DailyReviewGraphDebateContext
}

export interface DailyReviewGraphOutputSection {
  title: string
  content: string
}

export interface DailyReviewGraphNodeOutput {
  summary: string
  sections: DailyReviewGraphOutputSection[]
  markdown: string
}

export interface DailyReviewGraphNodePosition {
  x: number
  y: number
}

export interface DailyReviewGraphDebateContext {
  debate_id: string
  round: number
  turn: number
  max_rounds: number
  ...
  previous_argument_summary: string
  completed_turns: number
}

export interface DailyReviewGraphDebateTurn {
  id: string
  round: number
  turn: number
  speaker_id: string
  ...
  status: DailyReviewGraphNodeStatus
  attempt: number
  input: DailyReviewGraphDebateContext | null
  output: DailyReviewGraphNodeOutput | null
  error: string | null
  started_at: string | null
  completed_at: string | null
}

export interface DailyReviewGraphNode {
  id: string
  label: string
  kind: 'input' | 'analyst' | 'researcher' | 'manager' | 'planner' | 'risk' | 'agent' | 'synthesis'
  team_id: string
  description: string
  position: DailyReviewGraphNodePosition
  status: DailyReviewGraphNodeStatus
  attempt: number
  input: DailyReviewGraphNodeInput | null
  output: DailyReviewGraphNodeOutput | null
  error: string | null
  started_at: string | null
  completed_at: string | null
  turns: DailyReviewGraphDebateTurn[]
}

export interface DailyReviewGraphEdge {
  source: string
  target: string
  label: string
  kind: 'flow' | 'evidence' | 'debate' | 'feedback'
  dependency: boolean
  status: DailyReviewGraphEdgeStatus
}

export interface DailyReviewGraphGroup {
  id: string
  label: string
  description: string
  order: number
...
export interface DailyReviewGraphEvent {
  id: string
  sequence: number
  node_id: string
  node_label: string
...
export interface DailyReviewGraphProgress {
  completed: number
  total: number
  percent: number
...
export interface DailyReviewGraphDefinitionInput {
  id: string
  label: string
  source: string
  required: boolean
...
export interface DailyReviewGraphDefinitionOutput {
  id: string
  label: string
...
export interface DailyReviewGraphDefinitionNode {
  id: string
  label: string
  kind: DailyReviewGraphNode['kind']
  team_id: string
  description: string
  position: DailyReviewGraphNodePosition
  prompt: {
  ...
  required_inputs: DailyReviewGraphDefinitionInput[]
  required_outputs: DailyReviewGraphDefinitionOutput[]
}

export interface DailyReviewGraphDebateDefinition {
  id: string
  label: string
  participants: string[]
...
export interface DailyReviewGraphDebate extends DailyReviewGraphDebateDefinition {
  status: DailyReviewGraphNodeStatus
  completed_turns: number
  current_round: number
  current_speaker_id: string | null
  history: DailyReviewGraphDebateTurn[]
  error: string | null
  started_at: string | null
  completed_at: string | null
}

export interface DailyReviewGraphDefinition {
  schema_version: number
  framework: string
  mode: 'research_only'
  execution_enabled: boolean
  description: string
  debates: Record<string, DailyReviewGraphDebateDefinition>
  groups: DailyReviewGraphGroup[]
  nodes: DailyReviewGraphDefinitionNode[]
  edges: Array<Omit<DailyReviewGraphEdge, 'status'>>
}

export interface DailyReviewAnalysisGraph {
  schema_version: number
  framework: string
  mode: 'research_only'
  id: string
  ...
  target_ref: string
  symbol: string
  status: DailyReviewItemStatus
  debates: Record<string, DailyReviewGraphDebate>
  groups: DailyReviewGraphGroup[]
  nodes: DailyReviewGraphNode[]
  edges: DailyReviewGraphEdge[]
  events: DailyReviewGraphEvent[]
  progress: DailyReviewGraphProgress
  updated_at: string
}

export interface DailyReviewStrategyMatch {
  id: string
  name: string
  rank: number
  score: number | null
...
export interface DailyReviewScopeSummary {
  position_count: number
  priced_position_count: number
  missing_price_count: number
  stale_price_count: number
...
export interface DailyReviewPosition {
  account_id: string
  account_name: string
  source_ref: string
  symbol: string
...
export interface DailyReviewCandidate {
  ...
  asset_type: 'stock'
  rank: number
  matched_strategies: DailyReviewStrategyMatch[]
  reason: string
  news?: DailyReviewNewsItem[]
  status: DailyReviewItemStatus
  graph: DailyReviewAnalysisGraph
  report_id: string | null
  error: string | null
  report: AiStockReport | null
}

export interface DailyReviewRoutine {
  id: string
  ...
  news_context: {
    status: DailyReviewItemStatus | 'skipped'
    source_status: 'pending' | 'running' | 'completed' | 'degraded' | 'failed' | 'skipped'
    as_of?: string
    cutoff_at?: string
    unknown_timestamp_policy?: 'excluded'
    items: DailyReviewNewsItem[]
    item_count: number
    errors: string[]
  }
  strategy_screening: {
    status: DailyReviewItemStatus | 'skipped'
    selection_source: 'screener_pool' | 'all_available' | 'legacy'
    strategy_ids: string[]
  ...
  scope_summary: DailyReviewScopeSummary
  candidates: DailyReviewCandidate[]
  positions: DailyReviewPosition[]
  created_at: string
  updated_at: string
}

export interface DailyReviewRoutineSummary {
  id: string
  business_date: string
  run_number: number
  status: DailyReviewStatus
...
export interface DailyReviewNewsItem {
  id: string
  title: string
  summary: string
  snippet: string
...
export interface MonitorRule {
  ...
  // 分时自定义价位穿越: {symbol: price} (signal_intraday_price_cross_up/down 专用)
  intraday_price_levels?: Record<string, number>
...
export interface TradeFeeProfile {
...
export interface Preferences {
  ...
  trade_fee_profile: TradeFeeProfile
}
...
export const api = {
  ...
  updateTradeFeeProfile: (body: {
    commission_rate?: number | null
    min_commission?: number | null
    stamp_tax_rate?: number | null
    transfer_fee_rate?: number | null
  }) =>
    request<{ trade_fee_profile: TradeFeeProfile }>('/api/settings/preferences/trade-fee-profile', {
      method: 'PUT',
  ...
  portfolioWatchPool: () =>
    request<{ items: PortfolioWatchItem[] }>('/api/portfolio/watch-pool'),
  portfolioWatchPoolAdd: (symbol: string) =>
    request<PortfolioWatchItem>('/api/portfolio/watch-pool', {
      method: 'POST',
      body: JSON.stringify({ symbol }),
    }),
  portfolioWatchPoolRemove: (symbol: string) =>
    request<{ ok: boolean }>(
      `/api/portfolio/watch-pool/${encodeURIComponent(symbol)}`,
      { method: 'DELETE' },
    ),
  portfolioAccounts: () =>
    request<{ items: PortfolioAccount[] }>('/api/portfolio/accounts'),
  portfolioAccountCreate: (name: string) =>
    request<PortfolioAccount>('/api/portfolio/accounts', {
      method: 'POST',
      body: JSON.stringify({ name }),
    }),
  portfolioAccountUpdate: (accountId: string, name: string) =>
    request<PortfolioAccount>(`/api/portfolio/accounts/${encodeURIComponent(accountId)}`, {
      method: 'PATCH',
      body: JSON.stringify({ name }),
    }),
  portfolioAccountDelete: (accountId: string) =>
    request<{ ok: boolean }>(`/api/portfolio/accounts/${encodeURIComponent(accountId)}`, {
      method: 'DELETE',
    }),
  portfolioTrades: (accountId?: string, dateFrom?: string, dateTo?: string) => {
    const params = new URLSearchParams()
    if (accountId) params.set('account_id', accountId)
    if (dateFrom) params.set('date_from', dateFrom)
    if (dateTo) params.set('date_to', dateTo)
    const query = params.toString()
    return request<{ items: PortfolioTrade[] }>(`/api/portfolio/trades${query ? `?${query}` : ''}`)
  },
  portfolioTradeCreate: (body: {
    account_id: string
    symbol: string
    trade_date: string
    side: 'buy' | 'sell'
  ...
  // quiet: 409 等错误由调用方 catch 后自行 toast，避免与 request 内置 toast 重复
  portfolioTradeReorder: (tradeIds: string[]) =>
    request<{ ok: boolean }>('/api/portfolio/trades/reorder', {
      method: 'POST',
      body: JSON.stringify({ trade_ids: tradeIds }),
      quiet: true,
    }),
  // quiet: 错误由调用方 catch 后自行 toast，避免与 request 内置 toast 重复
  portfolioTradeUpdateExecution: (
    tradeId: string,
    body: { quantity: number; price: number },
  ) => request<PortfolioTrade>(`/api/portfolio/trades/${encodeURIComponent(tradeId)}`, {
    method: 'PATCH',
    body: JSON.stringify(body),
    quiet: true,
  }),
  // quiet: 错误由调用方 catch 后自行 toast，避免与 request 内置 toast 重复
  portfolioTradeUpdatePrice: (tradeId: string, price: number) =>
    request<PortfolioTrade>(`/api/portfolio/trades/${encodeURIComponent(tradeId)}/price`, {
      method: 'PATCH',
  ...
  portfolioTradeEstimate: (body: {
    symbol: string
    side: 'buy' | 'sell'
    quantity: number
    price: number
  }) =>
    request<TradeEstimateResult>('/api/portfolio/trades/estimate', {
      method: 'POST',
      body: JSON.stringify(body),
    }),
  portfolioStatementPreview: (accountId: string, file: File) => {
    const fd = new FormData()
    fd.append('account_id', accountId)
    fd.append('file', file)
    return request<StatementPreviewResponse>('/api/portfolio/statement-preview', {
      method: 'POST',
      body: fd,
    })
  },
  portfolioStatementCommit: (body: { account_id: string; items: StatementCommitItem[] }) =>
    request<StatementCommitResult>('/api/portfolio/statement-commit', {
      method: 'POST',
      body: JSON.stringify(body),
    }),
  portfolioSnapshot: (asOf: string, accountId?: string) => {
    const params = new URLSearchParams({ as_of: asOf })
    if (accountId) params.set('account_id', accountId)
    return request<PortfolioSnapshot>(`/api/portfolio/snapshot?${params}`)
  },
  portfolioPriceMonitors: () =>
    request<{ items: PortfolioPriceMonitor[] }>('/api/portfolio/price-monitors'),
  portfolioPriceMonitorSave: (symbol: string, body: {
    name: string
    asset_type: 'stock' | 'etf'
    stop_loss_price: number
    add_position_price?: number | null
    webhook_channels?: string[]
  }) => request<PortfolioPriceMonitor>(
    `/api/portfolio/positions/${encodeURIComponent(symbol)}/price-monitor`,
    { method: 'PUT', body: JSON.stringify(body), quiet: true },
  ),
  dailyReviewList: () =>
    request<{ items: DailyReviewRoutineSummary[]; running_count: number }>(
      '/api/daily-review/routines',
    ),
  dailyReviewGet: (routineRef: string) =>
    request<{ routine: DailyReviewRoutine | null }>(
      `/api/daily-review/routines/${encodeURIComponent(routineRef)}`,
    ),
  dailyReviewGraphDefinition: () =>
    request<DailyReviewGraphDefinition>('/api/daily-review/graph-definition'),
  dailyReviewRun: (businessDate: string, strategyIds: string[]) =>
    request<DailyReviewRoutine>(
      `/api/daily-review/routines/${encodeURIComponent(businessDate)}/run`,
      { method: 'POST', body: JSON.stringify({ strategy_ids: strategyIds }) },
    ),
  dailyReviewRetry: (routineId: string) =>
    request<DailyReviewRoutine>(
      `/api/daily-review/routines/${encodeURIComponent(routineId)}/retry`,
      { method: 'POST' },
    ),
  dailyReviewGraphRetry: (
    routineId: string,
    body: { target_type: 'position' | 'candidate'; source_ref: string; node_id: string },
  ) => request<DailyReviewRoutine>(
    `/api/daily-review/routines/${encodeURIComponent(routineId)}/graph/retry`,
    { method: 'POST', body: JSON.stringify(body) },
  ),
  dailyReviewInterruptAll: () =>
    request<{ interrupted_count: number; routine_ids: string[] }>(
      '/api/daily-review/routines/interrupt-all',
      { method: 'POST' },
    ),
  ...
  tradingDates: () => request<TradingDatesResponse>('/api/data/trading-dates'),
  ...
  stockAnalysisReportSave: (r: {
    ...
    levels?: Record<LevelType, PriceLevel[]>
    source?: 'portfolio'; source_ref?: string
  }) =>
  ...
  async *stockAnalyzeStream(
    symbol: string,
    focus?: string,
    target?: { accountId: string; asOf?: string },
  ): AsyncGenerator<{
    type: 'meta' | 'delta' | 'error' | 'done'
    symbol?: string
    summary?: string
    ...
    const path = target
      ? `/api/portfolio/accounts/${encodeURIComponent(target.accountId)}/positions/${encodeURIComponent(symbol)}/analyze`
      : '/api/stock-analysis/analyze'
    const res = await fetch(path, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(target
        ? { focus: focus ?? '', as_of: target.asOf ?? null }
        : { symbol, focus: focus ?? '' }),
    })
    if (!res.ok) {
      let detail = ''
