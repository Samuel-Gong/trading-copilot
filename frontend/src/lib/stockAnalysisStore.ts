...
export async function findTodayReport(symbol: string): Promise<HistoryReport | null> {
  ...
  return history.find(r => !r.source && r.symbol === symbol && (r.created_at ?? '').slice(0, 10) === today) ?? null
}
...
export async function startAnalysis(
  ...
  name: string,
  focus = '',
  target?: { accountId: string; sourceRef: string; asOf?: string },
): Promise<{ id?: string; error?: string }> {
  const existing = activeTasks.find(t =>
    (t.sourceRef ?? `stock:${t.symbol}`) === (target?.sourceRef ?? `stock:${symbol}`)
    && t.asOf === target?.asOf
    && (t.phase === 'loading' || t.phase === 'streaming'))
  if (existing) {
  ...
  runStream(id, symbol, name, focus, target)
  return { id }
}
...
async function runStream(
  ...
  try {
    let firstDelta = true
    for await (const chunk of api.stockAnalyzeStream(symbol, focus, target)) {
    ...
    if (final && final.phase !== 'error') {
      ...
      try {
        const res = await api.stockAnalysisReportSave({
          ...
          ...(final.source && final.sourceRef ? {
            source: final.source,
            source_ref: final.sourceRef,
          } : {}),
        })
        if (res.report) {
...
export async function retryAnalysis(task: {
  ...
  const target = task.accountId && task.sourceRef
    ? { accountId: task.accountId, sourceRef: task.sourceRef, asOf: task.asOf }
    : undefined
  return startAnalysis(task.symbol, task.name, task.focus, target)
}
