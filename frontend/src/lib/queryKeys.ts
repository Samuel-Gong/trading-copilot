...
export const QK = {
  ...
  // 不用 watchlist- 前缀: 日K历史盘中几乎不变, 避免被 quotes_updated 高频失效。
  watchlistKlineBatch:  (symbols: string) => ['kline-batch', symbols] as const,
  portfolioWatchPool:   ['portfolio-watch-pool'] as const,
  portfolioAccounts:    ['portfolio-accounts'] as const,
  portfolioTrades:      (accountId?: string) =>
                          ['portfolio-trades', accountId ?? 'all'] as const,
  portfolioSnapshot:    (asOf: string, accountId?: string) =>
                          ['portfolio-snapshot', asOf, accountId ?? 'all'] as const,
  portfolioPriceMonitors: ['portfolio-price-monitors'] as const,
  dailyReviewList:      ['daily-review', 'list'] as const,
  dailyReview:          (routineId: string) => ['daily-review', 'routine', routineId] as const,
  dailyReviewGraphDefinition: ['daily-review-graph-definition'] as const,
  ...
  screenerStrategyPool: ['screener-strategy-pool'] as const,
  ...
  tradingDates:         ['data-status', 'trading-dates'] as const,
