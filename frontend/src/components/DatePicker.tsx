...
interface DatePickerProps {
  ...
  enforceAvailableDatesFrom?: string
  disabled?: boolean
}
...
export function DatePicker({
  ...
  const isDateDisabled = (dateStr: string) => (
    ...
    || (
      ...
      && (!enforceAvailableDatesFrom || dateStr >= enforceAvailableDatesFrom)
  ...
  for (let i = offset - 1; i >= 0; i--) {
    ...
    cells.push({ day: d, cur: false, dateStr: ds, disabled: isDateDisabled(ds) })
  }
  // 当月
  for (let d = 1; d <= daysInMonth; d++) {
    const ds = toDateStr(viewYear, viewMonth, d)
    cells.push({ day: d, cur: true, dateStr: ds, disabled: isDateDisabled(ds) })
  }
  // 下月头部 — 补齐到 6 行 × 7 = 42
