...
# 本地开发进程必须直接联网，不继承启动 shell 或桌面应用注入的系统代理。
# NO_PROXY 仅固定回环地址，保证前端到本机 FastAPI 的请求不会被外部代理接管。
...
UVICORN_ARGS=(python -m uvicorn app.main:app)
if [[ -f "$ROOT/.env" ]]; then
  UVICORN_ARGS+=(--env-file "$ROOT/.env")
fi
...
(
  ...
  uv run --no-sync "${UVICORN_ARGS[@]}" --reload \
...
(
  ...
  BACKEND_HOST="$BACKEND_HOST" BACKEND_PORT="$BACKEND_PORT" \
    TICKFLOW_BACKEND_PORT="$BACKEND_PORT" \
