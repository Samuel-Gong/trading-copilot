...
- 前端布局与交互以常用桌面端宽屏为验收范围；在该范围内检查文字截断、遮挡、滚动区域和弹窗可操作性，不要求窄屏或移动端适配。
...
7. **界面运行检查**：有可见变化时实际检查修改后的交互、加载、空态、错误态和浅色/深色主题，并在 PR 中记录检查范围与结果；不提交截图、录屏或其他视觉附件，除非用户在本次需求中明确要求。
