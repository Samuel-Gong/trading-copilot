...
def _fake_repo(data_dir) -> types.SimpleNamespace:
    history = _history()
    ...
    return types.SimpleNamespace(
        ...
        get_enriched_history_snapshot=lambda: ("generation-1", history),
        get_enriched_range=get_enriched_range,
        store=types.SimpleNamespace(data_dir=data_dir),
    )


@pytest.fixture
def repo(monkeypatch, tmp_path) -> types.SimpleNamespace:
    map_df = pl.DataFrame(
        {"_sym_up": ["S1.SH", "S2.SH"], "concept": list(_MEMBERS)},
        schema={"_sym_up": pl.Utf8, "concept": pl.Utf8},
    ).with_columns(
        pl.lit("synthetic").alias("_source_id"),
        pl.lit(_LATEST - timedelta(days=_HISTORY_DAYS)).alias("_effective_date"),
    )
    monkeypatch.setattr(
