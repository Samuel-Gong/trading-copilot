...
def _route_fuyao(monkeypatch, provider):
    ...
    monkeypatch.setattr(custom, "registry_generation", lambda: 0)
    @contextmanager
    def lease(name):
        yield provider, 0
    monkeypatch.setattr(custom, "lease_provider", lease)
...
@pytest.mark.parametrize("change", ["route", "generation"])
def test_streaming_daily_rejects_source_change_before_publish(monkeypatch, repo, change):
    """流式取数期间切换路由或配置时，旧结果不得进入正式分区。"""
    from app.data_providers import custom

    selected = {"name": "fuyao"}

    def chunks():
        yield _daily("000001.SZ", date(2020, 1, 2))
        if change == "route":
            selected["name"] = "tickflow"
        else:
            monkeypatch.setattr(custom, "registry_generation", lambda: 1)

    provider = _StreamingProvider(chunks())
    _route_fuyao(monkeypatch, provider)
    monkeypatch.setattr(kline_sync.preferences, "get_daily_data_provider", lambda: selected["name"])
