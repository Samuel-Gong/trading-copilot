...
def test_codex_process_env_forces_configured_proxy_and_excludes_application_secrets(
    monkeypatch,
    tmp_path,
):
    ...
    for name in (
        ...
        monkeypatch.setenv(name, "http://inherited-proxy.invalid:7890")
    monkeypatch.setenv("NO_PROXY", "inherited.invalid")
    monkeypatch.setenv("no_proxy", "inherited.invalid")
    ...
    monkeypatch.setattr(ai_provider.settings, "codex_proxy_url", "http://127.0.0.1:7890")
    ...
    assert env["NO_PROXY"] == "127.0.0.1,localhost,::1,host.docker.internal"
...
def test_codex_process_env_rejects_missing_proxy(monkeypatch, tmp_path):
    monkeypatch.setattr(ai_provider.settings, "codex_proxy_url", "")

    with pytest.raises(RuntimeError, match="CODEX_PROXY_URL"):
        ai_provider._codex_process_env(tmp_path / "codex-home")


@pytest.mark.parametrize("proxy_url", ["127.0.0.1:7890", "file:///tmp/proxy", "ftp://proxy.example"])
def test_codex_process_env_rejects_invalid_proxy(monkeypatch, tmp_path, proxy_url):
    monkeypatch.setattr(ai_provider.settings, "codex_proxy_url", proxy_url)
