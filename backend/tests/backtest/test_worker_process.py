...
def _queue_mining_run(data_dir, start: date, run_id: str) -> dict:
    ...
    manifest = store.create(request, _mining_fingerprint(data_dir, request), run_id=run_id)
    return {
        "run_id": manifest["run_id"],
        "request": manifest["request"],
        "data_fingerprint": manifest["data_fingerprint"],
...
def test_mining_rejects_snapshot_when_generation_commits_mid_read(
...
def test_mining_fails_with_guidance_when_generation_keeps_drifting(
    ...
    service, strategy_engine = _mining_runtime_services(data_dir)
    state = _patch_generation_drift(monkeypatch, data_dir, always=True)
