...
async def test_backfill_rejects_bad_range_and_no_trading_days(tmp_path):
    with pytest.raises(ValueError, match="晚于"):
        await backfill_history(_saved_cfg(tmp_path), tmp_path, date(2026, 1, 9), date(2026, 1, 5))
    with pytest.raises(ValueError, match="上限"):
        await backfill_history(_saved_cfg(tmp_path), tmp_path, date(2025, 1, 1), date(2026, 9, 1))
    with pytest.raises(ValueError, match="交易日"):
        await backfill_history(_saved_cfg(tmp_path), tmp_path, date(2026, 1, 5), date(2026, 1, 9))
...
async def test_backfill_writes_partitions_and_is_idempotent(tmp_path, fake_http):
    ...
    result = await backfill_history(_saved_cfg(tmp_path), tmp_path, date(2026, 1, 1), date(2026, 1, 9))
    ...
    again = await backfill_history(_saved_cfg(tmp_path), tmp_path, date(2026, 1, 1), date(2026, 1, 9))
...
async def test_backfill_collects_failures_without_abort(tmp_path, fake_http):
    ...
    result = await backfill_history(_saved_cfg(tmp_path), tmp_path, date(2026, 1, 5), date(2026, 1, 6))
...
async def test_backfilled_partition_feeds_signal_frame(tmp_path, fake_http):
    ...
    await backfill_history(_saved_cfg(tmp_path), tmp_path, date(2026, 1, 5), date(2026, 1, 6))
...
async def test_backfill_429_retries_once_and_succeeds(tmp_path, fake_http, monkeypatch):
    ...
    result = await backfill_history(_saved_cfg(tmp_path), tmp_path, date(2026, 1, 5), date(2026, 1, 5))
...
async def test_backfill_aborts_after_consecutive_429(tmp_path, fake_http, monkeypatch):
    ...
    result = await backfill_history(_saved_cfg(tmp_path), tmp_path, date(2026, 1, 5), date(2026, 1, 9))
...
async def test_backfill_404_counts_as_empty_not_failed(tmp_path, fake_http, monkeypatch):
    ...
    result = await backfill_history(_saved_cfg(tmp_path), tmp_path, date(2026, 1, 5), date(2026, 1, 7))
