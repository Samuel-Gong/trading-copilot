...
- 财报历史按 (symbol, period_end) 聚合，沿用 upstream 的最新非空字段口径;
...
def attach_fundamental_factors(
    ...
    for name in missing_columns:
        ...
        if spec["price_ratio"]:
            value = pl.when(source > 0).then(pl.col("close") / source).otherwise(None)
        else:
            value = source
        expressions.append(
...
def build_fundamental_matrices(
    ...
    无数据为 NaN。pb 类因子在矩阵侧沿用 upstream 的 close / bps 口径。
    """
    requested = [str(name) for name in names if str(name) in FUNDAMENTAL_FACTOR_NAMES]
    if not requested:
        return {}
    ...
    announce_dates = snapshot["_announce"].to_list()
