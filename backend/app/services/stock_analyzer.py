...
def _load_kline(repo, symbol: str, as_of: date | None = None) -> pl.DataFrame:
    ...
    for table in ("metrics", "income"):
        ...
        if as_of is not None:
            announcement_column = next(
                (column for column in _ANNOUNCEMENT_DATE_COLUMNS if column in df.columns),
                None,
            )
            if announcement_column is None:
                # 没有公告日期就不能证明该财务事实在历史复盘日已经公开。
                out[table] = []
                continue
            df = df.filter(
                pl.col(announcement_column).cast(pl.Date, strict=False) <= as_of
            )
            if df.is_empty():
                out[table] = []
                continue
...
def _build_user_prompt(
    ...
    if has_fin:
        parts.extend([
            "",
            "以下是截至分析日已经公告的财务数据(JSON,核心指标 + 利润表,金额单位为元):",
            "```json",
    ...
    if portfolio_context:
        parts.extend([
            "",
            "以下是服务端读取的真实研究目标上下文。它只用于解释候选身份或账户暴露。"
            "不得据此输出买卖、加减仓、止损、目标价或仓位建议:",
            "```json",
...
def build_stock_analysis_input(
    ...
    """装配单 Agent 与多 Agent 共用的数据事实,不执行模型调用。"""
    df = _load_kline(repo, symbol, as_of)
    if df.is_empty():
        raise LookupError(f"标的 {symbol} 暂无日 K 数据,请先同步")

    levels = compute_levels(df)
    close = float(df.tail(1)["close"][0]) if "close" in df.columns else None
    fins = _load_financials(data_dir, symbol, as_of)
    user_prompt = _build_user_prompt(
        _clean_rows(df, _KLINE_KEEP_COLS),
        fins,
        levels,
        close,
        symbol,
...
async def analyze_stock_stream(
    ...
    """
    try:
        analysis_input = build_stock_analysis_input(
            repo,
            data_dir,
            symbol,
            focus,
