...
def _codex_process_env(codex_home_path: Path) -> dict[str, str]:
    """Pass a minimal environment and force Codex external requests through its proxy."""
    proxy_url = settings.codex_proxy_url.strip()
    try:
        parsed_proxy = urlsplit(proxy_url)
        valid_proxy = parsed_proxy.scheme in {"http", "https"} and bool(parsed_proxy.hostname)
    except ValueError:
        valid_proxy = False
    if not valid_proxy:
        raise RuntimeError("Codex CLI 代理未配置或格式无效, 请设置 CODEX_PROXY_URL")
    ...
    for name in _CODEX_ENV_ALLOWLIST:
        ...
        if value:
            env[name] = value
            seen.add(normalized)
    for name in _CODEX_PROXY_ENV_NAMES:
        env[name] = proxy_url
