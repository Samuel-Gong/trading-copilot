...
class QuoteService:
    ...
    @staticmethod
    def _attach_monitor_last_price(df: pl.DataFrame) -> pl.DataFrame:
        """把实时快照的 close 显式暴露为监控专用的分时最新价。

        QuoteService 的实时落库入口已把供应商 last_price 映射为当日 close；
        这里仅在监控边界恢复清晰命名，避免把历史日线收盘价与盘中触发价混为一谈。
        """
        if df.is_empty() or "last_price" in df.columns or "close" not in df.columns:
            return df
        return df.with_columns(pl.col("close").alias("last_price"))
        ...
        try:
            ...
            if self._app_state:
                ...
                if engine and engine.rule_count > 0:
                    ...
                    if stock_ready:
                        eval_df = self._attach_monitor_last_price(enriched_today)
                        if engine.has_rule_type("ladder"):
                            eval_df = self._inject_sealed_vol(eval_df, enriched_date)
                    ...
                    if engine.has_asset_rules("etf") and self._repo is not None:
                        try:
                            ...
                            if not etf_enriched.is_empty():
                                etf_enriched = self._attach_monitor_last_price(etf_enriched)
                    ...
                    if engine.has_asset_rules("index") and self._repo is not None:
                        try:
                            ...
                            if not index_enriched.is_empty() and index_date == cn_today():
                                index_enriched = self._attach_monitor_last_price(index_enriched)
    ...
    def _inject_intraday_signals(self, enriched: pl.DataFrame, engine, asset_type: str) -> pl.DataFrame:
        ...
        get_price_levels = getattr(engine, "intraday_price_levels", None)
        price_levels = get_price_levels(asset_type) if callable(get_price_levels) else None
