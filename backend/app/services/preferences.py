...
def load() -> dict:
    """读取 preferences.json，命中签名缓存时返回缓存内容的深拷贝。"""
    with _update_lock:
...
# ===== 实盘记账费率配置 =====
# 用于录交易时留空费用/税费的自动估算,金额最终以交割单导入校准为准。

_DEFAULT_TRADE_FEE_PROFILE = {
    "commission_rate": 0.00025,    # 佣金率(双边), 万 2.5
    "min_commission": 5.0,         # 单笔最低佣金(元); 0 表示免五
    "stamp_tax_rate": 0.0005,      # 印花税, 仅卖出股票收取
    "transfer_fee_rate": 0.00001,  # 沪市股票过户费(双边), 深市与 ETF 不收
}


def get_trade_fee_profile() -> dict:
    """返回实盘记账费率配置,缺失字段回落到默认口径。"""
    profile = load().get("trade_fee_profile")
    merged = dict(_DEFAULT_TRADE_FEE_PROFILE)
...
def set_trade_fee_profile(profile: dict) -> dict:
    """合并保存费率配置并返回生效值。"""
    current = get_trade_fee_profile()
    for key in current:
        raw = profile.get(key)
        if isinstance(raw, (int, float)) and not isinstance(raw, bool) and raw >= 0:
            current[key] = float(raw)
    save({"trade_fee_profile": current})
    return current
