...
INTRADAY_SIGNAL_LABELS: dict[str, str] = {
    ...
    "signal_intraday_price_above": "分时价格高于自定义价位",
    "signal_intraday_price_below": "分时价格低于自定义价位",
}

# 需要规则级 intraday_price_levels 配置的信号 (自定义价位条件)
PRICE_CROSS_FIELDS = frozenset({
    "signal_intraday_price_above",
...
def uses_price_cross_signals(rule: dict) -> bool:
    """规则是否使用了自定义价位条件信号 (需要 intraday_price_levels 配置)。"""
    return any(
        c.get("op") == "truth" and c.get("field") in PRICE_CROSS_FIELDS
        for c in rule.get("conditions", [])
        if isinstance(c, dict)
    )
