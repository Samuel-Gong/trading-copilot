...
OPS = {">", ">=", "<", "<=", "==", "!="}
# 监控运行时额外提供的分时字段。不能放进 custom_signals.ALLOWED_FIELDS，
# 因为自定义日线信号没有分时最新价输入。
THRESHOLD_FIELDS = frozenset(ALLOWED_FIELDS) | {"last_price"}
...
def validate(rule: dict) -> None:
    ...
    # 自定义价位穿越信号: 需要 intraday_price_levels 配置, 且价位标的必须在 symbols 中
    if uses_price_cross_signals(rule):
        levels = rule.get("intraday_price_levels")
        if not isinstance(levels, dict) or not levels:
            raise ValueError("自定义价位穿越信号需要配置 intraday_price_levels (标的→价位)")
        syms = set(rule.get("symbols", []))
        missing = set(levels.keys()) - syms
        if missing:
            raise ValueError(f"intraday_price_levels 包含未在 symbols 中的标的: {sorted(missing)}")
        for sym, price in levels.items():
            if not isinstance(price, (int, float)) or price <= 0:
                raise ValueError(f"intraday_price_levels[{sym}] 必须是正数")
...
def normalize(rule: dict) -> dict:
    ...
    r.setdefault("intraday_price_levels", {})
    # ladder 专属默认字段
