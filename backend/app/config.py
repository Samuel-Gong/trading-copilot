...
class Settings(BaseSettings):
    ...
    # 系统内的 Codex CLI 子进程必须通过独立代理访问外网, 不继承后端进程的代理。
    codex_proxy_url: str = "http://127.0.0.1:7890"
    ...
    # 每日复盘新闻证据。采用 trading-copilot-bak / daily_stock_analysis 中的
    # NewsNow 默认源组合；抓取失败时复用本地归档并 fail-open。
    review_news_enabled: bool = True
    newsnow_base_url: str = "https://newsnow.busiyi.world"
    review_news_source_ids: str = (
        "cls-hot,xueqiu-hotstock,wallstreetcn-quick,jin10,gelonghui"
    )
    review_news_timeout_seconds: float = 5.0
    review_news_max_items_per_source: int = 20
