...
def get_preferences() -> dict:
    ...
    return {
        ...
        **preferences.get_realtime_quote_scope(),
        ...
        "trade_fee_profile": preferences.get_trade_fee_profile(),
    }
...
class TradeFeeProfileIn(BaseModel):
    commission_rate: float | None = Field(default=None, ge=0, le=0.05, allow_inf_nan=False)
    min_commission: float | None = Field(default=None, ge=0, le=1000, allow_inf_nan=False)
    stamp_tax_rate: float | None = Field(default=None, ge=0, le=0.05, allow_inf_nan=False)
    transfer_fee_rate: float | None = Field(default=None, ge=0, le=0.05, allow_inf_nan=False)


@router.put("/preferences/trade-fee-profile")
def update_trade_fee_profile(req: TradeFeeProfileIn) -> dict:
    """保存实盘记账费率配置,留空字段保持现值。"""
    from app.services import preferences
    updates = {key: value for key, value in req.model_dump().items() if value is not None}
    saved = preferences.set_trade_fee_profile(updates)
    return {"trade_fee_profile": saved}
