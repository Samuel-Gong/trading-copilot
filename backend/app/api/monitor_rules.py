...
def sync_engine(request: Request) -> None:
...
class RuleModel(BaseModel):
    ...
    intraday_price_levels: dict[str, float] = {}
...
def get_options(request: Request):
    ...
    # 阈值字段 (带中文标签)
    monitor_field_labels = {"last_price": "分时最新价"}
    threshold_fields = [
        {"key": f, "label": monitor_field_labels.get(f, ENRICHED_COLUMNS.get(f, f))}
        for f in sorted(monitor_rules.THRESHOLD_FIELDS)
    ]
