...
def _build_series(df: pl.DataFrame) -> dict:
    ...
    这些指标的本质是"每日一条线",随 MA/ATR/标准差漂移,画成曲线才能体现通道形态。
    ...
    if has_atr:
        ...
        # ATR 止损/止盈: close 加减 2 倍 ATR(跟随行情漂移的动态止损线)
        out["atr"] = {
...
class SaveReportRequest(BaseModel):
    ...
    source: Literal["portfolio"] | None = None
...
def save_report(request: Request, req: SaveReportRequest):
    ...
    if req.source_ref:
        payload["source_ref"] = req.source_ref
    report = stock_reports.save_report(payload)
