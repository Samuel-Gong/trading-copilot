...
- **建仓前观察**:观察池只接受当前未持仓的 A 股和交易所 ETF。加入时同步进入自选,可以在观察池行内直接创建监控规则；任一账户形成非零持仓后自动移出观察池,但保留自选和已有监控规则。
- **交易驱动持仓**:多账户记录 A 股和交易所 ETF 的买入、卖出、成交价、手续费和税费；持仓按所选交易日回放，使用 FIFO 汇总剩余成本、已实现盈亏和浮动盈亏。
- **持仓价格监控**:持仓行快速设置必填止损价和可选加仓价，以连续竞价时段的分时最新价触发；同一证券跨账户共享规则，复用监控中心的冷却与通知链路，停用止损后持仓页会重新提示。
- **历史持仓分析**:从历史持仓行调用现有个股分析流程,补充所选日期的账户与成本上下文,不另建分析引擎。
- **按交易日冻结**:默认选择本地最近交易日，屏蔽周末、节假日和缺少 enriched 数据的日期；启动每日复盘时只回放不晚于业务日期的交易并固化持仓和估值范围，之后补录交易不会改写历史档案。
- **复盘历史账本**:左侧导航按交易日归组展示每一次运行、启动时间、状态和分析进度；同一交易日可以多次复盘，每次运行都使用独立 ID、序号、冻结范围、报告和研究 Graph，不覆盖同日旧记录。
- **中断全部运行**:只要存在进行中的每日复盘，页头会显示运行数量和“中断全部”。该操作同时取消所有活跃复盘协程及其市场、策略筛选和 Agent 子任务，并把尚未结束的节点持久化为“已中断”，之后可以按实例从 checkpoint 继续。
- **严格三步归档**:按“市场环境 → 策略候选 → 持仓分析”顺序执行；前一步未完成时后续步骤显式阻塞，不会提前启动。
- **可核对的策略候选**:每日复盘冻结“选股”页当前策略池，在历史行情上运行最新策略定义；空策略池保留零候选，不自动改用全部策略。候选按命中策略数、策略内排名倒数和证券代码排序，最多保留 8 个，并展示逐策略命中依据。
- **Point-in-time 新闻**:参考 `trading-copilot-bak` 接入 NewsNow 财联社、雪球热门、华尔街见闻、金十和格隆汇源并本地归档。历史复盘只注入发布时间不晚于复盘日的新闻，未知时间和未来新闻严格排除。
- **TradingAgents 风格研究 Graph**：候选与持仓共用 13 节点拓扑。四类 Analyst 完成后，Bull 与 Bear 按 `Bull₁ → Bear₁ → Bull₂ → Bear₂` 交替辩论，再进入研究裁决、研究方案和三类风险审查。流程停在 Portfolio Manager，不连接 Execution。
- **可观察数据流**：页面显示真实节点和边状态；运行中的辩论方向和依赖边显示动态数据流。点击 Bull、Bear 或 Research Manager 可查看完整辩论轮次、每次尝试、结构化输入输出和错误记录。
- **Agent 定义页**:“设置 → 分析 Agent”展示同一份静态拓扑、实际 Prompt 和输入输出要求,不会启动分析。
- **节点级恢复**：成功节点和已完成的辩论发言继续作为 checkpoint 保留；服务重启后未完成节点标记为中断。用户可以从指定失败节点恢复，也可以一次重试全部失败项。

操作步骤、估值口径、数据位置和明确非目标见[持仓与每日复盘使用指南](./user/portfolio-daily-review.md)。
