...
```ini
AI_PROVIDER=openai_compat              # openai_compat | codex_cli | ollama
AI_BASE_URL=https://api.deepseek.com/v1
AI_API_KEY=                            # 留空 = 关闭 AI
AI_MODEL=deepseek-chat
AI_DAILY_TOKEN_BUDGET=500000           # 每日 token 预算上限
CODEX_PROXY_URL=http://127.0.0.1:7890  # 系统内 Codex CLI 专用代理
```

| 配置项 | 说明 |
| :--- | :--- |
| `AI_PROVIDER` | `openai_compat`(OpenAI 兼容,支持 DeepSeek / 通义 / OpenAI 等)、`codex_cli` 或 `ollama`(本地模型) |
...
| `CODEX_PROXY_URL` | 系统内执行 `codex exec` 时强制使用的代理；未配置会拒绝调用。默认 `http://127.0.0.1:7890`，Docker Compose 自动改为 `http://host.docker.internal:7890` |
...
```ini
REVIEW_NEWS_ENABLED=true
NEWSNOW_BASE_URL=https://newsnow.busiyi.world
REVIEW_NEWS_SOURCE_IDS=cls-hot,xueqiu-hotstock,wallstreetcn-quick,jin10,gelonghui
REVIEW_NEWS_TIMEOUT_SECONDS=5
REVIEW_NEWS_MAX_ITEMS_PER_SOURCE=20
```

默认源与 `daily_stock_analysis` / `trading-copilot-bak` 的 NewsNow 组合一致。抓取结果保存在 `DATA_DIR/user_data/review_news.json`。历史复盘按新闻自身的 `published_at` 截断，不使用抓取时间替代发布时间；没有发布时间或晚于复盘日的条目不会进入 AI 上下文。单个源失败不会阻断复盘，错误会显示在“市场环境 → 新闻证据”。

`REVIEW_NEWS_SOURCE_IDS` 可从默认列表中删减；`REVIEW_NEWS_ENABLED=false` 会停止联网抓取，但仍可读取已有本地归档。
