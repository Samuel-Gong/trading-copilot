...
导出包含所选策略的完整当前命中结果，不含今日曾命中但已失效的股票；页面的临时筛选、排序和显示条数不影响导出。CSV 每个策略命中一行，同一股票命中两个策略时保留两行及各自评分；TXT 和 JSON 的 `symbols` 跨策略去重。

升级后首次导出如果提示缓存缺少股票日线标记，点击「重载」重新运行策略。旧缓存仍可在原页面读取，但导出需要明确的资产与周期标记，避免旧版本 ETF 运行留下的结果混入股票。
...
接口复用页面的已完成结果，包含监控引擎的最新股票日线结果。每次请求读取当前快照，不触发策略计算，不提供历史归档。所选策略必须全部有结果且日期一致，否则整批拒绝，不返回不完整清单。盘中结果可能变化；需要盘后清单时，应在每日选股完成后请求并由接入软件保存。

指定 `as_of` 时，仅用同日期的监控结果覆盖磁盘快照；若监控已进入下一交易日，仍可导出磁盘中尚未被替换的匹配日期结果。
